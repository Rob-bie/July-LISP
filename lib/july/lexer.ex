defmodule July.Lexer do
  ...
  # (special forms)
  # Tokenize a keyword (q'|'|~|if|cond|match|fun|defun|def|let||>|show|<>|list|import)
  ...
  defp tokenize([?m, ?a, ?t, ?c, ?h, c|rest], token_acc, tokens, line_number) do
    case c do
      c when c in '\s\r\t[]()' ->
        tokenize([c|rest], token_acc, [{:keyword, "match", line_number}|tokens], line_number)
      c when c in '\n' ->
        tokenize(rest, token_acc, [{:keyword, "match", line_number}|tokens], line_number + 1)
      _ ->
        symbol_chars(rest, [c, ?h, ?c, ?t, ?a, ?m |token_acc], tokens, line_number)
    end
  end
  ...
  defp tokenize([?s, ?h, ?o, ?w, c|rest], token_acc, tokens, line_number) do
    case c do
      c when c in '\s\r\t[]()' ->
        tokenize([c|rest], token_acc, [{:keyword, "show", line_number}|tokens], line_number)
      c when c in '\n' ->
        tokenize(rest, token_acc, [{:keyword, "show", line_number}|tokens], line_number + 1)
      _ ->
        symbol_chars(rest, [c, ?w, ?o, ?h, ?s |token_acc], tokens, line_number)
    end
  end
